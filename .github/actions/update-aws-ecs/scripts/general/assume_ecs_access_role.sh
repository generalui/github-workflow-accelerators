...
# `source ./assume_ecs_write_access_role.sh --roleArn "arn:aws:iam::123456789012:role/ecs-access"`

# Function to handle options and arguments
assume_ecs_write_access_role() {
    ...
    usage() {
        ...
        echo >&2 -e "${BLUE} -r, --roleArn       The ARN of the role to assume to get ECS access${NC}"
    }
    ...
    if [ "$just_usage" = false ]; then
        ...
        source "$SCRIPT_PATH"/assume_role.sh --roleArn "$role_arn" --sessionName "ecs-access"
    fi
}
